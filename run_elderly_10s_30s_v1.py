#!/usr/bin/env python3
"""
Run the elderly pipeline from scratch for 10s and 30s windows.

Key difference from original:
- Adjusted tolerance_s for larger windows (10s needs 5s, 30s needs 15s)
- Only runs ONE window size at a time to avoid confusion
"""
...
# Tolerance mapping: window_sec -> tolerance_s
# Rule: tolerance should be ~50% of window length to handle sensor offsets
TOLERANCE_MAP = {
    5.0: 2.0,
    10.0: 5.0,
    30.0: 15.0,
...
def generate_config(subject, data_root, window_sec):
    """Generate pipeline config for a specific subject and window size."""
    ...
    # Get proper tolerance for this window size
    tolerance = TOLERANCE_MAP.get(window_sec, window_sec / 2.0)
    ...
    config = {
        ...
        "windowing": {
            "overlap": 0.1,
            "window_lengths_sec": [window_sec],  # Only the specified window
        },
        "features": {
            "imu_bioz": {
                "modality": "imu",
        ...
        "fusion": {
            ...
            "window_lengths_sec": [window_sec],
            "tolerance_s": {
                str(window_sec): tolerance,  # Use proper tolerance!
            },
            "modalities": {
...
def run_subject_pipeline(subject, data_root, window_sec):
    """Run pipeline for a single subject at a specific window size."""
    print(f"\n{'='*70}")
    print(f"SUBJECT: {subject} | WINDOW: {window_sec}s")
    print(f"{'='*70}")
    
    # Generate config
    config = generate_config(subject, data_root, window_sec)
    if not config:
        print(f"✗ Could not generate config for {subject}")
        return False
    
    # Write config to temporary file
    config_path = Path(f"/tmp/pipeline_{subject}_{window_sec}s.yaml")
    with open(config_path, "w") as f:
        yaml.dump(config, f)
    
    print(f"✓ Config: {config_path}")
    print(f"  Tolerance: {TOLERANCE_MAP.get(window_sec, window_sec/2)}s")
...
def combine_datasets(subjects_config, window_sec):
    """Combine aligned fused features from multiple subjects."""
    print(f"\n{'='*70}")
    print(f"COMBINING DATASETS (window={window_sec}s)")
    print(f"{'='*70}")
    ...
    for subject, cfg in subjects_config.items():
        ...
        aligned_path = (
            ...
            / f"elderly_{subject}"
            / f"fused_aligned_{window_sec:.1f}s.csv"
        )
...
def train_xgboost(df_labeled, pruned_cols, output_dir, window_sec):
    ...
    print(f"\n{'='*70}")
    print(f"TRAINING: XGBoost with GroupKFold CV ({window_sec}s)")
    print(f"{'='*70}")
    ...
    # Save results
    results_dir = output_dir / f"xgboost_results_{window_sec}s"
    ...
    return model, r, rmse, mae


def train_ridge(df_labeled, pruned_cols, output_dir, window_sec):
    """Train Ridge Regression with proper GroupKFold CV."""
    from sklearn.model_selection import GroupKFold, cross_val_predict
    ...
    print(f"\n{'='*70}")
    print(f"TRAINING: Ridge Regression with GroupKFold CV ({window_sec}s)")
    print(f"{'='*70}")
    ...
    # Save results
    results_dir = output_dir / f"ridge_results_{window_sec}s"
    results_dir.mkdir(parents=True, exist_ok=True)
...
def run_full_pipeline(window_sec):
    """Run full pipeline for a specific window size."""
    print("="*70)
    print(f"ELDERLY PATIENTS PIPELINE - {window_sec}s WINDOWS")
    print(f"Subjects: sim_elderly3, sim_elderly4, sim_elderly5")
    print(f"Tolerance: {TOLERANCE_MAP.get(window_sec, window_sec/2)}s")
    print("="*70)
    ...
    succeeded = {}
    for subject, cfg in SUBJECTS_CONFIG.items():
        if run_subject_pipeline(subject, cfg["data_root"], window_sec):
            ...
            if aligned_path.exists():
                print(f"  ✓ Found partial data for {subject}, including it")
                succeeded[subject] = cfg
    ...
    # Combine datasets
    combined = combine_datasets(succeeded, window_sec)
    
    if combined is None or len(combined) == 0:
        print(f"\n✗ No data found at {window_sec}s!")
        return None
    ...
    combined_file = output_path / f"elderly_aligned_{window_sec:.1f}s.csv"
    combined.to_csv(combined_file, index=False)
    print(f"\n✓ Saved combined dataset: {combined_file}")
    
    # Feature selection
    print(f"\n{'='*70}")
    print(f"FEATURE SELECTION + QC ({window_sec}s)")
    print(f"{'='*70}")
    ...
    feature_qc_dir = output_path / f"qc_{window_sec:.1f}s"
    ...
    # Train models
    xgb_results = train_xgboost(df_labeled, pruned_cols, output_path, window_sec)
    ridge_results = train_ridge(df_labeled, pruned_cols, output_path, window_sec)
    
    # Summary stats
    print(f"\n{'='*70}")
    print(f"FINAL SUMMARY ({window_sec}s windows)")
    print(f"{'='*70}")
    ...
    print(f"\nResults:")
    print(f"  XGBoost: r={xgb_results[1]:.3f}, RMSE={xgb_results[2]:.2f}, MAE={xgb_results[3]:.2f}")
    print(f"  Ridge:   r={ridge_results[1]:.3f}, RMSE={ridge_results[2]:.2f}, MAE={ridge_results[3]:.2f}")
    ...
    return {
        ...
        "n_features": len(pruned_cols),
        "xgboost_r": xgb_results[1],
        "xgboost_rmse": xgb_results[2],
        "xgboost_mae": xgb_results[3],
        "ridge_r": ridge_results[1],
        "ridge_rmse": ridge_results[2],
        "ridge_mae": ridge_results[3],
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Elderly pipeline for specific window size")
    parser.add_argument(
        "--window",
        type=float,
        required=True,
        choices=[5.0, 10.0, 30.0],
        help="Window size in seconds (5, 10, or 30)",
    )
    args = parser.parse_args()
    
    results = run_full_pipeline(args.window)
